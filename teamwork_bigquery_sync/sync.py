...
def run_dry_run(client):
    ...
    # TEMPORARY spike diagnostic (2026-09-03): testing a specific claim
    # (from an external LLM, not verified against docs or the real API) —
    # that `showCompletedLists=true` (plus `status=all`) is the real flag
    # for surfacing tasks in a completed tasklist, on both tasks.json and
    # the tasklist-scoped tasks endpoint. Also testing `status=all` alone
    # as its own candidate. Target: task 49325131 in completed tasklist
    # 3941748 (project 1388473), confirmed real and reachable only by
    # single-task GET as of the last investigation.
    print("\n--- showCompletedLists claim-check (temporary spike) ---")
    ...
    for label, params in (
        ("no flag", {}),
        ("showCompletedLists=true", {"showCompletedLists": "true"}),
        ("status=all", {"status": "all"}),
        (
            "status=all&showCompletedLists=true",
            {"status": "all", "showCompletedLists": "true"},
        ),
    ):
        try:
    ...
    # Also check the claimed tasklists.json?status=all reconciliation step.
    try:
        payload = client._get(
            f"/projects/api/v3/projects/{target_project_id}/tasklists.json", {"status": "all"}
        )
        tasklists = payload.get("tasklists", [])
        match = next((tl for tl in tasklists if tl.get("id") == target_tasklist_id), None)
        print(
            f"     GET projects/{target_project_id}/tasklists.json (status=all): "
            f"{len(tasklists)} tasklist(s), target present: {match is not None}"
        )
    except Exception as exc:
        print(f"     GET projects/{target_project_id}/tasklists.json (status=all): FAILED — {exc}")
