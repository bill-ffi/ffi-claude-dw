...
An offline pytest suite lives in `teamwork_bigquery_sync/tests/` — 406 tests, no network, no BigQuery, no credentials. Run it with `pip install -r requirements-dev.txt && python -m pytest tests/`; CI runs it on every push via `.github/workflows/tests.yml`. Every case corresponds to a bug this pipeline actually hit, so a failure means a real regression, not a style quibble. Add a test alongside any fix, and **mutation-test it**: re-introduce the bug and confirm the suite fails, because a test that passes against broken code is worse than none. That is how this suite was validated (nine past bugs originally, and each guard added since). Commit before mutating — restoring with `git checkout --` on uncommitted work destroys the fix you are testing.
