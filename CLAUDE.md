...
An offline pytest suite lives in `teamwork_bigquery_sync/tests/` — 408 tests, no network, no BigQuery, no credentials. Run it with `pip install -r requirements-dev.txt && python -m pytest tests/`; CI runs it on every push via `.github/workflows/tests.yml`. Every case corresponds to a bug this pipeline actually hit, so a failure means a real regression, not a style quibble. Add a test alongside any fix, and **mutation-test it**: re-introduce the bug and confirm the suite fails, because a test that passes against broken code is worse than none. That is how this suite was validated (nine past bugs originally, and each guard added since). Commit before mutating — restoring with `git checkout --` on uncommitted work destroys the fix you are testing.
...
- Running a real sync and inspecting the `RUN_SUMMARY` JSON log line at the end for per-table row counts and resolution stats (e.g. `rows_with_category_name`, `clients_resolved`, `activity.method`). Each stage also carries `fill_rates` and `underfilled_columns` for the columns in `schemas.ALWAYS_POPULATED_COLUMNS` — a non-empty `underfilled_columns`, or a `WARNING` naming a column below a 100% fill rate, means a column the pipeline just wrote is (partly) empty. This exists because both `web_link` columns sat at 0% for months while every run reported success; it warns, never fails. The summary also carries `unresolved_timelog_users` (timelog user ids with no `users` row, across all history; `[]` is healthy, `null` means the check could not run) — added after two former staff's time read nameless for months because `people.json` omits deleted people unless sent `showDeleted=true` (`teamwork_client.USERS_LIST_PARAMS`). A fill-rate check could not catch that: the id was always filled, only the join failed. Both full syncs and `--create-views` also carry `table_expirations` (the dataset's default expiration plus any table/view with a deletion date; `null`/`[]` is healthy) — added after the dataset was found to be deleting every table 60 days after creation, with the core tables a month from loss. A truncate-and-load does not reset a table's expiration. Keep that list conservative — adding a legitimately-sparse column makes the warning fire every run and trains everyone to ignore it.
...
Measured 2026-09-04: the expected `tasks` row count is **~37,226** (823 in-scope projects), made up of 13,930 tasks from 218 active projects plus 23,296 from 605 projects archived on/after the cutoff. **Re-measured 2026-09-14** via `v_task_review` (which filters to `archived_at IS NULL`): **14,595 tasks across 208 non-archived projects**, and zero tasks whose project row is missing. Ten projects were archived out and ~665 tasks net were added in ten days, so treat any single figure here as a dated snapshot with meaningful drift, not a constant — re-run `--explain-task-scope` rather than comparing against a number in this file. That the non-archived project count still matches the active-status count is a fresh confirmation the collinearity below still holds. `status` and `archived_at` are perfectly collinear on this account (`active` ⟺ never archived), so `ACTIVE_PROJECT_STATUSES` currently changes nothing. Note that `WHERE p.archived_at >= '2026-01-01'` measures only the archived half (23,296) and is *not* a check on the table's total size — see README "Known gaps" before concluding the scope is wrong.
...
`v_timelog_detail` is the other base view — one row per time entry, joined out to project, client, person, tasklist and Activity. Any new time-based report should read from it rather than re-joining `timelogs` to `projects`/`tasks`/`users`, so the join logic, the derived `hours` (computed as `minutes / 60`, since `timelogs.hours` is stored pre-rounded), and the `task_join_status` classification all stay defined in one place.
...
**Sixteen views in total**: seven `v_exception_*` rules, plus `v_usermins`, `v_user_daily_billable_hours_base`, `v_user_weekly_billable_hours`, `v_timelog_detail`, `v_task_review`, `v_project_detail`, `v_client_month`, `v_user_daily_time_split` and `v_data_freshness`.

`v_user_daily_time_split` is one row per logged date × user × client × Activity (plus `week_start`, the Sunday beginning the week, for Looker roll-ups) with hours split into `pto_hours` (all time on task `PTO_TASK_ID`, tested first so leave on the FFI client never counts as internal), `internal_hours` (all other time on company id `INTERNAL_CLIENT_COMPANY_ID`, FFI, billable or not — the client test deliberately precedes the billable test), `client_billable_hours`, `cnb_hours` (client non-billable, including a NULL billable flag) and `no_client_hours`; the five always sum to `total_hours`. `PTO_TASK_ID` is also what `LONG_ENTRY_EXEMPT_TASK_IDS` exempts — one definition of the PTO task. Internal is matched on `company_id`, never on client name, so a Teamwork rename cannot reclassify time.

`v_data_freshness` is the odd one out — one row, and the only view whose subject is the pipeline rather than the business. It's the "last updated at" source for Looker Studio, which renders a BigQuery `TIMESTAMP` in UTC and offers no timezone-aware conversion of its own; the `DATETIME(ts, REPORTING_TIMEZONE)` conversion therefore has to happen in SQL, not as a fixed-offset `DATETIME_SUB` in Looker. Its headline takes `MAX(synced_at)` **within** each table (because `timelogs` is a windowed replace, so an old row keeps an old stamp) and then `MIN` **across** the four (because a failed stage leaves one table behind, and `MAX` would claim a freshness the dashboard lacks). Staleness is parameterized by `DATA_FRESHNESS_STALE_AFTER_HOURS`, set to 18 rather than the cron's nominal 12 because GitHub delivers the two daily firings 9.5-15.1h apart — see README.
...
`v_project_detail` is the projects-side equivalent: one row per **active** project, sharing `v_task_review`'s `archived_at IS NULL` scope predicate so the two cannot disagree about which projects exist. It exists because the raw `projects` table is a poor Looker source — bare user ids instead of names, and a REPEATED `tag_ids` the connector cannot read. Note its `logged_hours` (from `timelogs`, 2026-01-01 onward) does **not** reconcile with `budget_used` (Teamwork's own tracking); they measure different things.

**Creation order matters.** `create_or_replace_views()` iterates the dict returned by `build_view_sql()` in insertion order, and BigQuery requires a referenced view to already exist. Four views read from others: `v_user_weekly_billable_hours` (from `v_user_daily_billable_hours_base` and `v_usermins`), and `v_exception_time_without_task`, `v_client_month` and `v_user_daily_time_split` (all from `v_timelog_detail`). Add a dependent view *after* what it reads, and add a test asserting the ordering — `tests/test_views.py` has examples.
