...
An offline pytest suite lives in `teamwork_bigquery_sync/tests/` — 386 tests, no network, no BigQuery, no credentials. Run it with `pip install -r requirements-dev.txt && python -m pytest tests/`; CI runs it on every push via `.github/workflows/tests.yml`. Every case corresponds to a bug this pipeline actually hit, so a failure means a real regression, not a style quibble. Add a test alongside any fix, and **mutation-test it**: re-introduce the bug and confirm the suite fails, because a test that passes against broken code is worse than none. That is how this suite was validated (nine past bugs originally, and each guard added since). Commit before mutating — restoring with `git checkout --` on uncommitted work destroys the fix you are testing.
...
`v_user_daily_time_split` is one row per logged date × user × client × Activity (plus `week_start`, the Sunday beginning the week, for Looker roll-ups) with hours split into `internal_hours` (all time on company id `INTERNAL_CLIENT_COMPANY_ID`, FFI, billable or not — the client test deliberately precedes the billable test), `client_billable_hours`, `cnb_hours` (client non-billable, including a NULL billable flag) and `no_client_hours`; the four always sum to `total_hours`. Internal is matched on `company_id`, never on client name, so a Teamwork rename cannot reclassify time.
